    ...
    PlotType dependenceType)
{
    ...
    {
        ShowMessage(" оличество точек магнитного пол€ и эффекта ’олла не совпадает! filterdataHelper");
        return;
    }
    ...
    // а разве размер пол€ внутри фильтрации мен€етс€?ќ_о
...
/*
    ...
    while(i<NumberOfPoints && tempOutB[i]<=0 ) ++i; // ищем где поле становитс€ положительным.
...
bool MagneticFieldDependence::extrapolateData(const int polinomPowForMagnetoResistance=4, const int polinomPowForHallEffect=4)
{
    ...
    ++i;
    // ищем где поле становитс€ положительным.
...
void MagneticFieldDependence::constructPlotFromTwoMassive(PlotType pt, DataKind dk,TLineSeries* s,TColor color)
{
    ...
    switch(pt) // определ€ем что выводить.
    {
    case HALL_EFFECT:
